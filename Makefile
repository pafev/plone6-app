...
stack-start:  ## Local Stack: Start Services
	...
	@echo "Now visit: http://plone-app.localhost"
...
build-acceptance-servers: ## Build Acceptance Servers
	...
	@docker build backend -t pafev/plone-app-backend:acceptance -f backend/Dockerfile.acceptance
	@echo "Build acceptance frontend"
	@docker build frontend -t pafev/plone-app-frontend:acceptance -f frontend/Dockerfile

.PHONY: start-acceptance-servers
start-acceptance-servers: build-acceptance-servers ## Start Acceptance Servers
	@echo "Start acceptance backend"
	@docker run --rm -p 55001:55001 --name plone-app-backend-acceptance -d pafev/plone-app-backend:acceptance
	@echo "Start acceptance frontend"
	@docker run --rm -p 3000:3000 --name plone-app-frontend-acceptance --link plone-app-backend-acceptance:backend -e RAZZLE_API_PATH=http://localhost:55001/plone -e RAZZLE_INTERNAL_API_PATH=http://backend:55001/plone -d pafev/plone-app-frontend:acceptance

.PHONY: stop-acceptance-servers
stop-acceptance-servers: ## Stop Acceptance Servers
	@echo "Stop acceptance containers"
	@docker stop plone-app-frontend-acceptance
	@docker stop plone-app-backend-acceptance
